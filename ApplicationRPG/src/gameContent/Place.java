...
public abstract class Place {
	...
	public void showInfo() {
		if(!visited) {
			UserInterface.getInstance().println(firstVisitText);
			visited = true;
		}
			...
			public void handleCommand(String command) {
				...
				for (String action : possibleActions) {
					if(action.equals(commandChunks[0])) {
						switch (action) {
							case "chasser" : UserInterface.getInstance().println("vous chassez");
											if(randomGenerator.nextBoolean()) {
												new Fight(new Ennemi("lièvre",4,0,4)).attack(player);
												UserInterface.getInstance().println("vous récupérez 1 unité de nourriture");
												player.addEquipement(new Food());
											} else {
												new Fight(new Ennemi("sanglier",8,4,0)).attack(player);
												UserInterface.getInstance().println("vous récupérez 3 unités de nourriture");
												player.addEquipement(new Food());
												player.addEquipement(new Food());
												player.addEquipement(new Food());
											}
						...
						switch (action) {
							case "acheter" :
								if(commandChunks.length < 2) {
									UserInterface.getInstance().println("Préciser objet.");
									return;
								}
								switch(commandChunks[1]) {
									case "anti-moustique" :
										if(player.getMoney() >= 15) {
											UserInterface.getInstance().println("Vous achetez un spray anti-moustique.");
											player.setMoney(player.getMoney() - 15);
											player.addEquipement(new MosquitoHandler(10));
										} else {
											UserInterface.getInstance().println("Vous êtes trop pauvre.");
										}
									...
									case "épée" :
										...
										if(player.getMoney() >= 25) {
											UserInterface.getInstance().println("Vous achetez une épée");
										...
										return;
									default :
										UserInterface.getInstance().println("impossible");
										return;
								}
							case "vendre" :
								if(commandChunks.length < 2) {
								...
								switch(commandChunks[1]) {
									case "nourriture" :
										...
										if(playerFood != null) {
											UserInterface.getInstance().println("Vous vendez de la nourriture.");
											player.setMoney(player.getMoney() + 5);
											playerFood.setDestroyed(true);
										} else {
											UserInterface.getInstance().println("Vous n'avez pas de nourriture à vendre.");
										}
										return;
									default :
										UserInterface.getInstance().println("impossible");
				...
				for (String action : possibleActions) {
					if(action.equals(commandChunks[0])) {
						switch (action) {
							...
							case "visiter" :
								UserInterface.getInstance().println("Vous vous promenez dans le village et  tomber sur une vieille femme affolée.");
								UserInterface.getInstance().println("\"Pitié ! Quelqu'un aidez moi ! Mon chat est perdu dans le marais ! Je suis allergique aux moustiques je ne peux pas aller le chercher !\"");
								
								foulitVillage.removePossibleAction("visiter");
								grimbaughSwamp.addPossibleAction("chercherChat");
								return;
							case "rendreChat" : 
								UserInterface.getInstance().println("\"Merci monsieur ! Merci ! Prenez ceci en retour !\"");
								
							    UserInterface.getInstance().println("Vous recevez le manteau moche de la mère Michelle.");
			...
			public void handleCommand(String command) {
				...
				for (String action : possibleActions) {
					if(action.equals(commandChunks[0])) {
						switch (action) {
							case "volerReserve" : 
											UserInterface.getInstance().println("Vous vous rapprochez sournoisement de la réserve quand, semblant surgir de nul part, le propriétaire vous interpelle dans votre dos.");
											UserInterface.getInstance().println("\"Hola aventurier !\"");
											UserInterface.getInstance().println("Vous vous retournez lentement. Le viel homme est connu pour sa verbe. Vous pouvez l'assomer tout de suite et prendre le risque qu'il appelle la garde ou tentez de le faire partir pour revenir à votre larcin.");
											...
											return;
							case "assomerFermier" :
											UserInterface.getInstance().println("Vous cacher Gérard dans la réserve avant de vous emparer de torches.");
							...
							case "ecouterFermier" :
											...
											UserInterface.getInstance().println("(utiliser ENTER pour faire défiler le texte)");
											
											UserInterface ui = UserInterface.getInstance();
											for(String phrase : text) {
												UserInterface.getInstance().println(phrase);
												ui.getLine();
											}
					...
					if(action.equals(commandChunks[0])) {
						switch (action) {
							case "chercherChat" : new Hunger(1).attack(player);
												  UserInterface.getInstance().println("Vous trouvez le chat de la mère Michelle !");
					...
					if(action.equals(commandChunks[0])) {
						switch (action) {
							...
							case "chercherSphere" : new Hunger(3).attack(player);
											  		UserInterface.getInstance().println("Vous trouvez les sphères de vision !");
						...
						switch (action) {
							...
							case "avancer" : MagicWall magicWall = new MagicWall();
											 ...
											 if(!magicWall.isThreatening()) {
												 UserInterface.getInstance().println("En regardant à travers la sphère de vision, vous voyer un monde vert vibrant d'étrange façon.\nVous voyez une balafre rouge flamboyant sur une zone de la tour.");
												 muytakTower.removePossibleAction("avancer");
												 muytakTower.addPossibleAction("entrer");
											}
										    return;
						    //fin du jeu
							case "entrer" : UserInterface.getInstance().println("Avançant une main dans la balafre, vous êtes absorbé dans la tour.\nVous êtes plongé dans le noir pendant de longues minutes puis vous voyez une lueur bleue. En vous rapprochant vous vous rendez compte que c'est un orbe énorme couvert d'inscriptions illisibles qui flotte.");
											UserInterface.getInstance().println("Subotai parle dans votre dos. \"Nous l'avons trouvé ! ... Il est énorme ! .... Et bien maintenant je n'ai plus besoin de toi !\"");
											new Fight(new Ennemi("guerrier",15,5,5)).attack(player);
											UserInterface.getInstance().println("Fin du jeu !");
